...
robotarium_path = '../robotarium-matlab-simulator/'; 

path = genpath(robotarium_path);
if(isempty(path))
    disp('WARNING: Cannot find robotarium-simulator directory.  This script should be run from the base directory.')
else
    addpath(path)
end

path = genpath(strcat(robotarium_path,'utilities'));
if(isempty(path))
...
path = genpath(strcat(robotarium_path,'patch_generation'));
if(isempty(path))
